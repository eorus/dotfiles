#!/usr/bin/env bash
# Ultra-lightweight DWM statusbar for vanilla DWM with xrdb
...
while true; do
    ...
    # --- Combine and set status (plain text, colors come from DWM schemes) ---
    STATUS="$BAT_TEXT | $CPU_TEXT | $VOL_TEXT | $DISK_TEXT | $TEMP_TEXT | $TIME"

    # Update DWM status bar
    xsetroot -name "$STATUS"

    # Optional: write to file for external use
